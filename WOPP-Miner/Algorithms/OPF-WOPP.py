...
# counter for frequent patterns when stability filtering is disabled
sopp_num = 0
...
def read_numeric_series(file_name: str) -> List[float]:
    data = []
...
def reset_state(min_support: float):
    global minsup, length, k, sopp_num, fre_num, fre_number
    global element_num, contrast_num, level_counts, runtime
    S.clear()
    Fmap.clear()
    allfrepattern.clear()
    minsup = min_support
    length = len(S)
    k = 1.0 / length if length else 0.0
    sopp_num = 0
    fre_num = 0
    fre_number = 0
    element_num = 0
...
def read_file(path: str, min_support: float = 4.0, *_ignored):
    global minsup, length, k, sopp_num, fre_num, fre_number, cand_num
...
def find():
    global cand_num
    cand_num += 2  # two length-2 candidates: [1,2] and [2,1]
...
def judge_fre(Cd: List[int], Z: List[float], sup_num: float, group: int):
    ...
    if sup_num >= minsup:
        ...
        cand_key = str(Cd)
        Fmap[cand_key] = [Z, [sup_num], [float(group)]]
        allfrepattern[cand_key] = sup_num
        sopp_num += 1


def find_m():
    global fre_num, fre_number, Fmap, contrast_num, level_counts
    if fre_number > 0:
        level_counts.append(fre_number)
...
def mine_window(window_data: List[float], min_support: float, window_id: int = 1):
    global S, length, k, runtime, total_operation_count
    reset_state(min_support)
    S.extend(window_data)
    length = len(S)
    k = 1.0 / length if length else 0.0
    ...
    find()
    find_m()
    total_operation_count += contrast_num
    runtime = time.perf_counter() - start

    window_level_results.append(level_counts.copy())
    stats = stats_dict()
    stats["window_id"] = window_id
    stats["level_counts"] = level_counts.copy()
    window_stats.append(stats)


def run(data: List[float], window_len: int | None = None, step: int | None = None,
        min_support: float = 12.0):
    global cand_num, total_operation_count
    window_level_results.clear()
    window_stats.clear()
    cand_num = 0
    total_operation_count = 0
    ...
    for i, (window_data, _start_idx) in enumerate(get_windows(data, window_len, step)):
        mine_window(window_data, min_support, window_id=i + 1)

    return window_level_results


def get_windows(data: List[float], window_len: int, step: int):
    n = len(data)
    for i in range(0, n - window_len + 1, step):
        yield data[i : i + window_len], i


def stats_dict():
    return {
        "candidate_generated_count": cand_num,
        "candidate_checked_count": cand_num,
        "fusion_count": max(cand_num - 2, 0),
        "support_calc_count": element_num,
        "total_frequent_count": fre_num,
        "runtime": runtime,
        "operation": total_operation_count,
    }
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Run OPF miner with WOPP window.")
    parser.add_argument(
        "dataset",
        nargs="?",
        default="AAPL.txt",
    ...
    parser.add_argument(
        "--min-sup", "--minsup", dest="min_sup", type=float, default=12.0,
    ...
    start = time.perf_counter()
    run(data, window_len=window_len, step=step, min_support=args.min_sup)
    duration = time.perf_counter() - start
    print(f"Execution time: {duration:.6f} seconds")
    print(f"Window count: {len(window_level_results)}")
    if window_level_results:
        print(f"Last window level counts: {window_level_results[-1]}")
    if window_stats:
        print(f"Last window stats: {window_stats[-1]}")
