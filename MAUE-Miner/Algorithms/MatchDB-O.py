...
# mineau = 5000
# maxtime = 3024000
# s1 BPIC15_5.xes range(300)-13782
# mineau = 1600
# maxtime = 3024000
...
'''一次性标记'''
def init_bool(sDB):
    return [1 for _ in range(len(sDB))]
...
def no_que(timestamp_list, n, bool_s, seq, pattern):
    addlist = []
    usearray = []  # 标记本轮已用结点
    sflag = 1  # 判断是否终止循环
    k = 0
    while k < len(seq) and sflag:
        ...
        while m < len(pattern.data) and pflag:
            while l < len(seq):
                if pattern.data[m][0] == seq[l].data[0] and bool_s[l]:
                    if m != 0:
                        # 如果不满足时间约束，应该回溯
                        gaptime_flag = time_ok(timestamp_list, n, usearray[-1], l)
                        #
...
def mainfun():
    ...
    starttime = time.time()
    sort_item = onepattern(seqs, neu, res, rmu)  # 1序列模式及位置索引
    # print(len(candi))
    ...
    while 1:
        ...
        while i < len(candi):
            ...
            j = 0
            while j < len(seqs):
                bool_s = init_bool(seqs[j])
                addlist = no_que(timestamp_s, j, bool_s, seqs[j], candi[i])
