...
def onepattern(seqs, neu, res, rmu):
    ...
    #     temp.sup = candione.get(str(sort_item[i]))[1]
    #     FP.append(temp)
    #     i += 1
    return sort_item
...
'''s连接'''
def sbacktracking(seqs, timestamp_list, neu, res, rmu, fp, sort_item):
    ooc_dict = dict()
    if fp.sup == 0:
        j = 0
        while j < len(seqs):
            addlist = newsup(timestamp_list, j, fp)
            ooc_dict[j] = addlist
            j += 1
        # 计算平均效用(EAU)和效用上限（RUEbound）
        eau, ruebound = average_utility(neu, res, rmu, ooc_dict, fp)
        if ruebound < mineau:
            return
        if eau >= mineau and not (len(fp.data) == 1 and type(fp.data[0]) == str):
    ...
    while i < len(sort_item):
        ...
        tmp = Candilist(temp)
        candi.append(tmp)
        sbacktracking(seqs, timestamp_list, neu, res, rmu, tmp, sort_item)
        i += 1

'''深度优先生成候选模式 DFjion+isincan+DFjionin'''
def DFjoin(seqs, timestamp_list, neu, res, rmu, sort_item):
    FPnum.append(len(FPItem))
    while len(FPnum) != 0:
        i = 0
        while i < len(FP):
            sbacktracking(seqs, timestamp_list, neu, res, rmu, FP[i], sort_item)
            i += 1
        FPnum.append(len(FPItem))
        FP.clear()
        i = FPnum.pop(0)
...
def mainfun():
    ...
    DFjoin(seqs, timestamp_s, neu, res, rmu, sort_item)
    endtime = time.time()
