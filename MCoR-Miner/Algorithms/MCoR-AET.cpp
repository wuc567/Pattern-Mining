...
void read_file()
{
	...
	string buff;
	file.open("msnbc2.txt", ios::in);   //open sequence file
	int i = 0;
...
void enumtree_AETsigma(string prefix, vector <char>& allsigma,//vector <char> & freq_sigma,
...
void main()
{
	...
	//寻找频繁单项
	//discover_frequent_sigma(allsigma,frequence, min_sup, freq_sigma, freq_frequence );
	//display(freq_sigma, freq_frequence);
	//寻找频繁2长度模式
	//discover_frequent_2pattern(freq_sigma, freq_frequence, min_sup,freq_2pt,freq_2freq,mingap, maxgap, prefix);
	//enumtree_allsigma(prefix, allsigma,mingap, maxgap, minconf);
	enumtree_AETsigma(prefix, allsigma, mingap, maxgap, min_sup);
