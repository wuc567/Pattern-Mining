...
vector <pattern > all_freq_pattern;//存储了所有的频繁模式
...
int ismax(string& str, vector <pattern >& Patterns)
{//返回是1，表明是最大模式，即该模式在序列中没有出现过。
	int len = Patterns.size();
	int flag = 1;
	for (int i = 0;i < len;i++)
	{
		string cur = Patterns[i].name;
		int result = cur.find(str);
		if (result == 0)
		{
			int len = str.size(), len1 = cur.size();
			if (len1 > len)
				return 0;
		}
	}
...
void find_max(vector <pattern> Patterns)
{//找最大共生模式
	int len = Patterns.size();
	int count = 0;
	for (int i = 0;i < len;i++)
	{
		int result = ismax(Patterns[i].name, CoP);
		if (result == 1)
		{
			Max_CoP.push_back(Patterns[i]);
			count++;
		}
	}
	//cout <<count<<endl;
...
void find_CoP(vector <pattern>& AllPatterns, vector <pattern>& CoP, string& prefix)
{//找最大共生模式
	int len = AllPatterns.size();
	int count = 0;
	for (int i = 0;i < len;i++)
	{
		string cur = AllPatterns[i].name;
		int result = cur.find(prefix);
		if (result == 0)
		{
			CoP.push_back(AllPatterns[i]);
			count++;
		}
	}
	//cout <<count<<endl;
...
int Mine_All_frequent(vector <char>& freq_sigma, int& mingap,
	int& maxgap, double& min_sup, string& prefix)
{
	for (int len = 0;len < freq_sigma.size();len++)
	{
		string item;
		item = freq_sigma[len];
		freArr[0].push_back(item);
		canArr[0].push_back(item);
		//freArr[0]=freq_sigma;
		//canArr[0]=freq_sigma;
	}
	int f_level = 1;
	...
	while (candidate.size() != 0)
	{
		...
		for (int i = 0;i < candidate.size();i++)
		{
			...
			if (sup >= min_sup)
			{
				freArr[f_level].push_back(cand);
				canArr[f_level].push_back(cand);
				if (cand.substr(0, prefix.length()) == prefix)
				{
					pattern tmp;
					tmp.name = cand;
					tmp.support = sup;
...
//void find_CoP(all_freq_pattern，CoP, prefix);//找共生模式
//find_max(CoP);//找最大共生模式
...
void main()
{
	...
	//挖掘所有频繁模式。
	int f_level = Mine_All_frequent(freq_sigma, mingap, maxgap, min_sup, prefix);
