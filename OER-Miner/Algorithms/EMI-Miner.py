...
def onepattern(seqs):
    ...
    for key in sorted(candione.keys()):
        if candione.get(key) >= minsup:
            ...
            FPItem.append(temp)
            # 情节规则
            # FEDict[tuple(map(tuple, [temp.data]))] = temp.sup
        else:
            i = 0
...
def mainfun():
    ...
    while 1:
        ...
        while i < len(candi):
            ...
            while j < len(seqs):
                ...
                # print(candi[i].data, candi[i].sup, len(addlist))
                j += 1
            if candi[i].sup >= minsup:
                ...
                # 情节规则
                # data_tuple = tuple(map(tuple, candi[i].data))

                # pre_epi = tuple(map(tuple, candi[i].prefix))
                # pre_sup = FEDict.get(pre_epi)
                # conf = candi[i].sup / pre_sup
                # print(conf)
                # if conf >= minconf:
                #     SERule.append([pre_epi, data_tuple])
                #     caserule.append([pre_epi, data_tuple, pre_sup, candi[i].sup, conf])
                    # 情节规则
                    # 仅用于强模型生成
                    # strongDict[data_tuple] = addlist
                # FEDict[data_tuple] = candi[i].sup
                # FERule.append([pre_epi, data_tuple])
                # 情节规则
                # 仅用于频繁模型生成
                # modelDict[data_tuple] = addlist
            i += 1
        sorilink()
        FP.clear()
        if cannum == len(candi):
    ...
    # print("强规则数：", len(SERule))
    # print("全部规则数：", len(FERule))
    # print(caserule)
    # 仅用于模型生成
    # process_model(modelDict, activity_chart_r, timestamp_list_r)
    # i = 0
    # while i < len(FPItem):
