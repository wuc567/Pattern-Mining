...
class Sanitizer2:
    ...
    def classify_pattern(self, util, minutil, epsilon, delta):
        ...
        if 0 <= diff <= delta:
            priority_level = "High"  # 最紧急：刚好超过阈值
        elif delta < diff <= epsilon:
            priority_level = "Moderate"  # 中等：有一定距离
        else:  # diff > epsilon
            priority_level = "Low"  # 低优先级：距离阈值较远
    ...
    def compute_epsilon_delta_by_utility(self, util_list, minutil, steps=3, delta_ratio=0.2):
        if not util_list:
            return 1, 1
    ...
    def select_sacrifice_sequence(self, meta, non_sensitive_patterns, seq_overlap=None, alpha=1.0):
        ...
        for occ in occs:
            ...
            score = 2*sensitive_contrib - overlap_penalty
