...
class Sanitizer2:
    ...
    def classify_pattern(self, util, minutil, epsilon, delta):
        ...
        if 0 <= diff <= delta:
            ...
            priority_level = "Z2"
        elif epsilon < diff <= 2*epsilon:
            priority_level = "Z3"
        elif epsilon < diff <= 3*epsilon:
            priority_level = "Z4"
        else:  # diff > epsilon
            priority_level = "Z5"
    ...
    def compute_epsilon_delta_by_utility(self, util_list, minutil, steps=5, delta_ratio=0.2):
        if not util_list:
            return 1, 1
    ...
    def _parse_pattern_itemS_entry(self, entry):

        if entry is None:
            return []

        if isinstance(entry, list) and all(not isinstance(x, dict) for x in entry):
            return entry

        if isinstance(entry, list) and all(isinstance(x, dict) for x in entry):
            max_seq = -1
            for e in entry:
                seq = e.get("seq", -1)
            ...
            normalized = [[] for _ in range(max_seq + 1)]
            for e in entry:
                seq = e.get("seq")
                pos = e.get("positions", [])
                normalized[seq] = pos
    ...
    def build_sensitive_meta(self, sensitive_patterns, mine_cache, epsilon, delta, minutil):
        ...
        for p in sensitive_patterns:
            ...
            meta.append({
                ...
                "priority_level": priority_level,  # 优先级等级
                "diff": diff,
                "priority": priority,
                "status": "pending",
                "occurrences": occurrences  # 现在这是一个list of {"seq": ..., "positions": ...}
            })
    ...
    def compute_prefix_overlap_ratio(self, sensitive_pattern, non_sensitive_patterns):
        if not non_sensitive_patterns:
            return 0.0

        ratios = []
        for nsp in non_sensitive_patterns:
            overlap_len = 0
            for i in range(min(len(sensitive_pattern), len(nsp))):
                if sensitive_pattern[i] == nsp[i]:
                    overlap_len += 1
                else:
                    break
            ratio = overlap_len / len(sensitive_pattern)
            ratios.append(ratio)
    ...
    def select_victim_items_by_partition(self, meta, priority_level, risu_values, dataset, non_sensitive_patterns,
        ...
        if priority_level == "Z1":
            pattern_risu = {x: risu_values.get(x, float("inf")) for x in pattern}
            chosen = min(pattern_risu, key=pattern_risu.get)

        elif priority_level == "Z2":
            best_item = None
            best_score = float("-inf")
            for item in pattern:
                nsc = sum(1 for t in non_sensitive_patterns if item in t) if non_sensitive_patterns else 0
                if nsc < best_score:
                    best_item = item
                    best_score = nsc
            chosen = best_item if best_item else pattern[-1]

        elif priority_level == "Z3":
            best_item = None
            best_freq = float("inf")

            for item in pattern:
                freq = sum(1 for t in dataset if item in t)
                if freq < best_freq:
                    best_freq = freq
                    best_item = item
        ...
        elif priority_level == "Z4":
            best_item = None
            best_score = float("-inf")
            for item in pattern:
                sc = sum(t.get(item, 0) for t in dataset if item in t)
                if sc > best_score:
                    best_item = item
                    best_score = sc
            chosen = best_item if best_item else pattern[-1]

        elif priority_level == "Z5":
            pattern_risu = {x: risu_values.get(x, float("inf")) for x in pattern}
    ...
    def _select_best_position_to_delete(self, victim_item, seq_id, S, pattern_positions, pattern):
        ...
        for occ_idx, occurrence in enumerate(pattern_positions):
            ...
            for victim_idx in victim_indices:
                ...
                pos = occurrence[victim_idx]  # 获取 victim_item 在这个 occurrence 中的位置
    ...
    def sanitize_patterns_by_partition(
        ...
        partition_priority = [
            "Z1",  # 0 ≤ diff ≤ delta (最高优先级)
            "Z2",  # delta < diff ≤ epsilon (中等优先级)
            "Z3",  # diff > epsilon (低优先级)
            "Z4",
            "Z5"
        ]
