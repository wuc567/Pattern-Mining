...
freq_sigma = []
freq_frequence = {}
def discover_frequent_sigma(new_allsigma, minsup, freq_sigma, new_frequence):
    for i in range(len(new_allsigma)):
        if new_frequence[new_allsigma[i]] >= minsup:
            t = new_allsigma[i]
            freq_sigma.append(t)
...
freq_2pt = []
def discover_frequent_2pattern(freq_sigma, minsup, freq_2pt, mingap, maxgap, prefix):
    len_fp = len(prefix)
    if len_fp == 1:  # 只处理长度为1的前缀，用于生成长度为2的情节
        for i in range(len(freq_sigma)):
            for j in range(len(freq_sigma)):  # 避免自身组合和重复组合i + 1, len(HU1)
                ti = freq_sigma[i]
                tj = freq_sigma[j]
                # 提取时间戳并转换为整数
                ti_char, ti_time = ti
