            ...
            for (Map.Entry<String, List<Integer>> entry : mapV2.entrySet()) {
                ...
                if (mapInNew != null) {
                    ...
                    for (Map.Entry<String, List<Integer>> entryIn : mapInNew.entrySet()) {
                        patternEnumNew(P, PLocation, entryIn, PNode);
...
//            LNode QNode = getLNode(entryIn.getValue());
    ...
    private static void patternEnumNew(Double[] P, List<Integer> PLocation, Map.Entry<String, List<Integer>> entryIn, LNode PNode) {
        int slen = P.length;
        String key = entryIn.getKey();
        Double[] r = stringToArray(key);
        for (int temp = 1; temp <= slen + 1; temp++) {
            ...
            Cd[slen] = (double) temp;
            Double[] Q = new Double[Cd.length - 1];
            // 求后缀
            System.arraycopy(Cd, 1, Q, 0, Q.length);

            Double[] q = getOrder(Q);
                List<Integer> postion=entryIn.getValue();
                //相对顺序相同
                if (Arrays.equals(q, r)) {
                    // 一样
                    // p[0] != q[max]
                    double A=P[0];
                    double B=r[r.length-1];
                    if (A!= B) {
                        grow_BaseP11(postion, PNode, Cd, false);
                    } else {
                        // p[0] == q[max]
                        if (Cd[0]< Cd[slen]) {
                            grow_BaseP22(slen, postion, PNode, 1, Cd, false);
                        } else {
                            grow_BaseP22(slen, postion, PNode, 2, Cd, false);
                        }
    ...
    private static void patternEnum(Double[] P, List<Integer> pList) {
        int slen = P.length;
        ...
        for (int temp = 1; temp <= slen + 1; temp++) {
            ...
            for (Map.Entry<String, List<Integer>> entry : map.entrySet()) {
                String key = entry.getKey();
                Double[] r = stringToArray(key);
                List<Integer> postion=entry.getValue();
                //相对顺序相同
                if (Arrays.equals(q, r)) {
                    // 一样
                    // p[0] != q[max]
                    ...
                    if (A!= B) {
                        ...
                        grow_BaseP11(postion, PNode, Cd, true);
                    } else {
                        // p[0] == q[max]
                        if (Cd[0]< Cd[slen]) {
                            grow_BaseP22(slen, postion, PNode, 1, Cd, true);
                        } else {
                            grow_BaseP22(slen, postion, PNode, 2, Cd, true);
                        }
    ...
    static void grow_BaseP11(List<Integer> postion,LNode L, Double[] Cd, boolean isFirst){
...
//                System.out.println("共生模式：" + Arrays.toString(Cd) + ", sup:" + Z.size() + ": " + Z.toString());
    ...
    static void grow_BaseP22(int slen, List<Integer> postion, LNode L, int flag, Double[] Cd, boolean isFirst){
