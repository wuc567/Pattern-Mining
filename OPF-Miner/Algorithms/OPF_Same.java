...
public class OPF_Same {
    ...
    private static void calculate() {
        if (fre_number > 0) {
            ...
            List<OPF_Same.LNode> Lb1 = new ArrayList<>();
            ...
            if (flag) {
                for (Map.Entry<String, List<List<Double>>> entry1 : sortedData) {
                    ...
                    for (Map.Entry<String, List<List<Double>>> entry2 : sortedData) {
                        OPF_Same.LNode QNode = Lb1.get(i);
        ...
        while (fre_number > 0) {
            ...
            Fmap = new LinkedHashMap<>();
            List<OPF_Same.LNode> Lb1 = new ArrayList<>();
            List<OPF_Same.LNode> Lb2 = new ArrayList<>();
            ...
            for (Map.Entry<String, List<List<Double>>> entry1 : sortedData) {
                ...
                if ((group == 1) || (group == 3)) {
                    ...
                    for (Map.Entry<String, List<List<Double>>> entry2 : G1) {
                        OPF_Same.LNode QNode = Lb1.get(i);  //Q位置
                        if (PNode.data >= minsup && QNode.data >= minsup) {
                            Integer[] Q = stringToArray(entry2.getKey());   //Q
                    ...
                    for (Map.Entry<String, List<List<Double>>> entry2 : G2) {
                        OPF_Same.LNode QNode = Lb2.get(i);  //Q位置
                        if (PNode.data >= minsup && QNode.data >= minsup) {
                            Integer[] Q = stringToArray(entry2.getKey());   //Q
                            Integer[] QPre = new Integer[Q.length - 1];  //获取Q前缀
                            System.arraycopy(Q, 0, QPre, 0, QPre.length);
    ...
    private static void patternFusion(Integer[] P, OPF_Same.LNode PNode, Integer[] Q, OPF_Same.LNode QNode, int group) {
        int slen = P.length;
        if (P[0] == Q[Q.length - 1]) {
    ...
    private static void grow_BaseP2(int slen, OPF_Same.LNode qNode, OPF_Same.LNode pNode, Integer[] Cd, Integer[] Cd2, int group) {
        List<Double> Z = new ArrayList<>();
        List<Double> Z2 = new ArrayList<>();
        int lst;
        OPF_Same.LNode p = pNode;
        OPF_Same.LNode q = qNode;
    ...
    private static void grow_BaseP1(OPF_Same.LNode qNode, OPF_Same.LNode pNode, Integer[] Cd, int group) {
        List<Double> Z = new ArrayList<>();
        OPF_Same.LNode p = pNode;
        OPF_Same.LNode q = qNode;
