...
public class OPF_noSuf {
    ...
    private static void calculate() {
        if (fre_number > 0) {
            ...
            List<OPF_noSuf.LNode> Lb1 = new ArrayList<>();
            ...
            if (flag) {
                for (Map.Entry<String, List<List<Double>>> entry1 : sortedData) {
                    ...
                    OPF_noSuf.LNode PNode = getLNode(PLocation);//p位置
                    ...
                    for (Map.Entry<String, List<List<Double>>> entry2 : sortedData) {
                        ...
                        if (PNode.data >= minsup) {
                            OPF_noSuf.LNode QNode = Lb1.get(i);
        ...
        while (fre_number > 0) {
            ...
            List<OPF_noSuf.LNode> Lb1 = new ArrayList<>();
            List<OPF_noSuf.LNode> Lb2 = new ArrayList<>();
                ...
                if ((group == 1) || (group == 3)) {
                    ...
                    for (Map.Entry<String, List<List<Double>>> entry2 : G1) {
                        ...
                        if (PNode.data >= minsup) {
                            ...
                            if (Arrays.equals(getOrder(PSuf), getOrder(QPre))) {
                                OPF_noSuf.LNode QNode = Lb1.get(i);  //Q位置
                    ...
                    for (Map.Entry<String, List<List<Double>>> entry2 : G2) {
                        ...
                        if (PNode.data >= minsup) {
                            ...
                            contrast_num++;//前缀后缀比较次数
                            if (Arrays.equals(getOrder(PSuf), getOrder(QPre))) {
                                OPF_noSuf.LNode QNode = Lb2.get(i);  //Q位置
    ...
    private static void patternFusion(Integer[] P, OPF_noSuf.LNode PNode, Integer[] Q, OPF_noSuf.LNode QNode, int group, List<Double> suffset, int index) {
        int slen = P.length;
        if (P[0] == Q[Q.length - 1]) {
    ...
    private static void grow_BaseP2(int slen, OPF_noSuf.LNode qNode, OPF_noSuf.LNode pNode, Integer[] Cd, Integer[] Cd2, int group, List<Double> suffset, int index) {
        List<Double> Z = new ArrayList<>();
        List<Double> Z2 = new ArrayList<>();
        int lst;
        OPF_noSuf.LNode p = pNode;
        OPF_noSuf.LNode q = qNode;
    ...
    private static void grow_BaseP1(OPF_noSuf.LNode qNode, OPF_noSuf.LNode pNode, Integer[] Cd, int group, List<Double> suffset, int index) {
        List<Double> Z = new ArrayList<>();
        OPF_noSuf.LNode p = pNode;
        OPF_noSuf.LNode q = qNode;
    ...
    private static OPF_noSuf.LNode getLNode(List<Double> List) {
        OPF_noSuf.LNode PNode = new OPF_noSuf.LNode();
        PNode.data = Double.valueOf(List.size());
        OPF_noSuf.LNode p = new OPF_noSuf.LNode();//p是节点
        OPF_noSuf.LNode s = new OPF_noSuf.LNode();//s是节点
        s = PNode;
        for (Double val : List) {
            p = new OPF_noSuf.LNode();
            p.data = val;
            s.next = p;
            s = p;
        }
