...
public class OPF_noPre {
    ...
    private static void calculate() {
        if (fre_number > 0) {
            ...
            List<OPF_noPre.LNode> Lb1 = new ArrayList<>();
        ...
        while (fre_number > 0) {
            ...
            List<OPF_noPre.LNode> Lb1 = new ArrayList<>();
            List<OPF_noPre.LNode> Lb2 = new ArrayList<>();
                ...
                if ((group == 1) || (group == 3)) {
                    ...
                    for (Map.Entry<String, List<List<Double>>> entry2 : G1) {
                        ...
                        if (suff >= minsup) {
                            ...
                            if (Arrays.equals(getOrder(PSuf), getOrder(QPre))) {
                                OPF_noPre.LNode QNode = Lb1.get(i);  //Q位置
                    ...
                    for (Map.Entry<String, List<List<Double>>> entry2 : G2) {
                        ...
                        if (suff >= minsup) {
                            ...
                            contrast_num++;//前缀后缀比较次数
                            if (Arrays.equals(getOrder(PSuf), getOrder(QPre))) {
                                OPF_noPre.LNode QNode = Lb2.get(i);  //Q位置
    ...
    private static void patternFusion(Integer[] P, OPF_noPre.LNode PNode, Integer[] Q, OPF_noPre.LNode QNode, int group, List<Double> suffset, int index) {
        int slen = P.length;
        if (P[0] == Q[Q.length - 1]) {
    ...
    private static void grow_BaseP2(int slen, OPF_noPre.LNode qNode, OPF_noPre.LNode pNode, Integer[] Cd, Integer[] Cd2, int group, List<Double> suffset, int index) {
        List<Double> Z = new ArrayList<>();
        List<Double> Z2 = new ArrayList<>();
        int lst;
        OPF_noPre.LNode p = pNode;
        OPF_noPre.LNode q = qNode;
        double f1=0.0;//减去的就是超模式的支持度
        double f2=0.0;//减去的就是超模式的支持度
        double f=suffset.get(index);//后缀支持度初始值
    ...
    private static void grow_BaseP1(OPF_noPre.LNode qNode, OPF_noPre.LNode pNode, Integer[] Cd, int group, List<Double> suffset, int index) {
        List<Double> Z = new ArrayList<>();
        OPF_noPre.LNode p = pNode;
        OPF_noPre.LNode q = qNode;
    ...
    private static OPF_noPre.LNode getLNode(List<Double> List) {
        OPF_noPre.LNode PNode = new OPF_noPre.LNode();
        PNode.data = Double.valueOf(List.size());
        OPF_noPre.LNode p = new OPF_noPre.LNode();//p是节点
        OPF_noPre.LNode s = new OPF_noPre.LNode();//s是节点
        s = PNode;
        for (Double val : List) {
            p = new OPF_noPre.LNode();
            p.data = val;
            s.next = p;
            s = p;
        }
