...
def DFjion(sort_item):
    FPnum.append(len(FPItem))
    while len(FPnum)!=0:
        i=0
        while i<len(FP):
            ibacktracking(FP[i],sort_item)
            sbacktracking(FP[i],sort_item)
            i+=1
        FPnum.append(len(FPItem))
        FP.clear()
        i = FPnum.pop(0)
...
'''i连接'''
def ibacktracking(fp,sort_item):
    if fp.sup==0:
        pre = splitpattern(fp)
        j = 0
        while j < len(seqs):
            splitlist(twolist, j, pre, fp)
            j += 1
    if fp.sup<minsup:
        return
    elif len(fp.data[0]) != 1 and type(fp.data[0])!=str :
    ...
    while i<len(sort_item):
        ...
        if i > lastidx:
            ...
            if type(temp[-1]) == str:
                ...
                tp=Candilist(temp)
        candi.append(tp)
        ibacktracking(tp, sort_item)
        i+=1

'''s连接'''
def sbacktracking(fp,sort_item):
    if fp.sup==0:
        pre = splitpattern(fp)
        j = 0
        while j < len(seqs):
