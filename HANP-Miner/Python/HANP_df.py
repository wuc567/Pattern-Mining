...
def nonoverlength():
	...
	print(len(nettree[0]),len(nettree[1]))
	store = 0
	for i in range(len(nettree[0])):
		position = nettree[0][i]
		if not position.toleave:
			continue
		a = position.max_leave - position.name + 1
		b = position.min_leave - position.name + 1
		if (minlen>a or a>maxlen) and (minlen>b or b>maxlen):
			position.used = True
			position.toleave = False
			continue
		occin = occurrence()
		for i in range(ptn_len+1):
			occin.position.append(None)
		occin.position[0] = i
		position.used = True
		position.toleave = False
		flag = True
		for j in range(ptn_len):
			for child in nettree[j][occin.position[j]].children:
				a1 = nettree[j + 1][child].max_leave - position.name + 1
				b1 = nettree[j + 1][child].min_leave - position.name + 1
				if nettree[j + 1][child].used == False and (a1 <= maxlen and a1 >= minlen) or (b1 >= minlen and b1 <=maxlen):
					occin.position[j + 1] = child
					nettree[j + 1][child].used = True
					nettree[j + 1][child].toleave = False
					flag = False
					break
		if flag:
			for k in range(j + 1):
				nettree[k][occin.position[k]].used = False
				nettree[k][occin.position[k]].toleave = True
...
def HANP_Miner(filename, mingap, maxgap, minsup):
	...
	output_filename = 'result_file_SARA.txt'
	with open(output_filename, 'w') as file:
		print('-------- Results of ', __file__.split('/')[-1].replace('.py', ''), ' for ',
		      filename.split('/')[-1].replace('.txt', ''), ' --------', file=file)
		for i in range(f_level):
			if len(freArr[i]) == 0:
