...
K = 600000  # The sequence number of sequence database
M = 100000  # The length of pattern  # define N 200000  //The length of sequence
...
def gen_candidate(level):
	...
	'''
	global candidate
	for cand_k in canArr[level - 1]:
		for cand_i in canArr[0]:
			cand = cand_k
			cand = cand + cand_i
...
def min_freItem():
	...
	for iterator in counter.items():
		...
		if list(iterator)[1] * hupval < minsup:
			...
			freArr[0].append(list(iterator)[0])
...
def clear_mem():
	...
	K = 600000  # The sequence number of sequence database
	M = 200000  # The length of pattern
	N = 600000  # The length of sequence
	S = ''
	unum = [0.0 for i in range(K)]
	ww = 0
	my_dict = dict()
...
def HANP_bf(filename, mingap, maxgap, minsup, output_filename):
	read_file(filename)
	cannum = 0
	frenum = 0
	compnum = 0
...
if (__name__ == '__main__'):
	...
	maxgap = 3
	minsup = 2500
	HANP_bf(filename, mingap, maxgap, minsup)