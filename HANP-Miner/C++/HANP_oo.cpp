...
//	  if (start<0)
	...
	for (k=kn;k<=kx;k++)
	{
		if (table[k-start].r[ptn_len-1]==1)
		{
			table[i-start].r[ptn_len]=1;
			break;
		}
	}
	if (table[i-start].r[ptn_len]==1)
	{
		//???????
		match[ptn_len]=i;
		for (j=ptn_len;j>0;j--)
		{
			kn=match[j]-sub_ptn[j-1].max -1;
			kx=match[ptn_len]-sub_ptn[j-1].min -1;
			if (kn<start)
				kn=start;
			for (k=kn;k<=kx;k++)
			{
