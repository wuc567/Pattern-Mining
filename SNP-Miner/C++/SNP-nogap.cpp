...
int binary_search(int level,string cand,int low,int high)
{
	...
	{
		mid=(high+low)/2;
		int result=cand.compare(freArr[level-1][mid].name.substr(0,level-1)); //To avoid multiple calls the same function
		if (result == 0)
		{
		    //find start
			int slow=low;
...
void gen_candidate(int level)
{
	...
	{
		string Q="",R="";
		R = freArr[level-1][i].name.substr(1,level-1);  //suffix pattern of freArr[level-1][i]
		Q = freArr[level-1][start].name.substr(0,level-1);   //prefix pattern of freArr[level-1][start]
		if(Q != R)
		{
	...
	vector<sorted_incomplete_nettree> *freArr)
{
	...
	for (int i = 0; i < sigmasize; i++)
	{
		...
		if (current_pattern.pos_sup >= minsup) //wyh
		{
...
void main()
{
	...
	for(int i = 0; i<f_level; i++)
	{
		...
		{
			cout<<freArr[i][j].name<<"\t"<<freArr[i][j].pos_sup;
			cout<<endl;
			frenum++;
		}
		cout<<endl;
