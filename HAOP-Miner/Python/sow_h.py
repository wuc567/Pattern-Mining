...
from common import seqdb, pant_p, readfile, Creat_ptn, binary_search


N  = 20000  # The length of sequence
...
def InitSequence(s: str, seq: list):
    seq.clear()
    for i in range(len(s)):
        seq.append({
            'c' : s[i], 
...
def matching(S: str, p : str, seq: list, occ: list):
    num = 0
    n = len(S)
    m = len(p)
    i = 0
    ...
    while j < m:
        flag = -1
        while i < n:
            if seq[i]['c'] == p[j] and seq[i]['used'] == False:
                flag = 1
                occ[j] = i
                seq[i]['used'] = True
                break
...
def sow_h(filename: str, minunity : float):
            ...
            for db in sdb:
                ...
                if len(db.S) > 0:
                    ...
                    if ptn_len > s_length:
                        ...
                        InitSequence(db.S, seq)
                        num = matching(db.S, p, seq, occ)
