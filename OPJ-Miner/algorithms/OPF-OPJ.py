...
def find_3():
    ...
    # 对长度为2的频繁模式进行降序排序，根据支持度
    sorted_FOP = sorted(FOP[-1], key=lambda opj: len(occ_dic[opj]), reverse=True)
    # 长度为2的OPP一定可以融合，但是它们的MLP不一定能连接
    for p in sorted_FOP:
        # p的mlp后缀
        mlp_suf = p[1][1:]
        for q in sorted_FOP:
            ...
            r, h = JPFusion(p, q)
            # 构建初始分组
            if p[0] == (1, 2) and q[0] == (1, 2):
                group_dic[r[0]] = 1
            elif p[0] == (1, 2) and q[0] == (2, 1):
                group_dic[r[0]] = 2
                group_dic[h[0]] = 2
            elif p[0] == (2, 1) and q[0] == (1, 2):
                group_dic[r[0]] = 3
                group_dic[h[0]] = 3
            else:
                group_dic[r[0]] = 4
...
def find_m():
    global occ_dic, cand_cnt, group_dic
    while FOP[-1]:
        # 增加长度为length的FOP结果集
        new_FOP = list()
        new_occ_dic = dict()
        ...
        G2 = []
        for p in sorted_FOP:
            if group_dic[p[0]] == 1 or group_dic[p[0]] == 2:
                G1.append(p)
            else:
                G2.append(p)
        # print(len(G1), len(G2))
        ...
        for p in sorted_FOP:
            ...
            mlp_suf = p[1][1:]
            # 13只能和G1融合，24只能和G2融合
            if group_dic[p[0]] == 1 or group_dic[p[0]] == 3:
                suffix_list = G1
            else:
                suffix_list = G2
            ...
            for q in suffix_list:
                ...
                if h:
                    ...
                    if len(occ_r) >= min_sup:
                        new_FOP.append(r)
                        new_occ_dic[r] = occ_r
                        new_group_dic[r[0]] = group_dic[p[0]]
                    if len(occ_h) >= min_sup:
                        new_FOP.append(h)
                        new_occ_dic[h] = occ_h
                        new_group_dic[h[0]] = group_dic[p[0]]
                else:
                    cand_cnt += 1
                    occ_r = cal_occ_1(p, q)
                    if len(occ_r) >= min_sup:
