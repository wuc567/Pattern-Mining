...
参数
'''
filename = "./Datasets/SDB2.txt"  # 数据集 文件路径/文件名
win = 500  # 窗口大小
minsup = 1200  # 最小支持度阈值
maxlen = 1  # 最长模式的长度
len_init = 0  # 初始数据库中序列个数
seqnum = 24026  # 数据库中序列总长度
step = 80  # 窗口每次滑动的长度
        ...
        while j < len(seqs[i]):
            ...
            while k < len(seqs[i][j].data):
                if seqs[i][j].data[k] not in candione.keys():
                    ...
                    addNode1(pattern, SupQueue(win), 1)
...
'''更新节点信息 支持度等'''
def updateNode(key,length):
    updateSup(Fmap[length][key])
    if Fmap[length][key].sup < minsup:
        return
    p=Fmap[length][key]
    if length==1:
        tp = copy.deepcopy(p.pattern[-1])
        for item in candione.keys():
            tmp = []
            ad=[]
            ad.append(item)
            if ad[-1] > tp[-1]:
                tmp.append(tp + ad)
                addNode(tmp,SupQueue(win),length+1)
                tmp = []
            tmp.append(tp)
            tmp.append(ad)
            addNode(tmp,SupQueue(win),length+1)
