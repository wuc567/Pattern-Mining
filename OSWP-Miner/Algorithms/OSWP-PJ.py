...
def deleteNode(p,length):
    if length==1:
        ...
        for key in Fmap[length].keys():
            ad = copy.deepcopy(Fmap[length][key].pattern)
            tmp = []
            if ad[-1][-1]>tp[-1][-1]:
                tmp.append(tp + ad)
                poplist[0][list_to_string(tmp)]=length+1
                poplist[1][list_to_string(tmp)] = length + 1
                if list_to_string(tmp) in Fmap[length + 1].keys():
                    deleteNode(Fmap[length + 1][list_to_string(tmp)],length+1)
                tmp = []
            tmp.append(tp)
            tmp.append(ad)
            poplist[0][list_to_string(tmp)] = length + 1
