...
def onepattern(seqs, Lminsup):
    ...
    while i < len(sort_item):
        ...
        i += 1
    return sort_item, candi, fre_patt, FPItem, FP
...
def client_episodes(seqs, timestamp_s, result):
    ...
    sort_item, candi, fre_patt, FPItem, FP = onepattern(seqs, Lminsup)  # 1序列模式及位置索引,0->1
...
def client_episodes_S(seqs, timestamp_s, csup):
    candinum = []  # 候选模式不同长度起始位置
    Lminsup = csup
    print('支持度阈值', Lminsup)
...
def client_episodes_one(seqs, timestamp_s, pattern):
    ...
    if isinstance(p[0], str):
        ...
        j = 0
        while j < len(seqs):
            addlist = one_scan(timestamp_s, j, seqs[j], pattern)
            j += 1
        useto0(seqs)
        # if p == [['d'], ['d']]:
        #     print(pattern.sup)
...
def duplicate_sublists(list1, num):
    list2 = [copy.deepcopy(sublist) for sublist in list1 for _ in range(num)]
    return list2
...
def mainfun():
    ...
    # flattened_list2 = duplicate_sublists(flattened_list, sup_num)
    clients = {1: [0, 1], 2: [1, 2], 3: [2, 3]}
    # clients = {1: [0, sup_num], 2: [sup_num, 2*sup_num], 3: [2*sup_num, 3*2*sup_num]}
    # Elog8
    ...
    # c1_FPItem, c1_candi, c1_Lminsup = client_episodes_S(seqs[clients.get(1)[0]:clients.get(1)[1]],
    #                                                   timestamp_s[clients.get(1)[0]:clients.get(1)[1]], 141*sup_num)
    ...
    # c2_FPItem, c2_candi, c2_Lminsup = client_episodes_S(seqs[clients.get(2)[0]:clients.get(2)[1]],
    #                                                   timestamp_s[clients.get(2)[0]:clients.get(2)[1]], 161*sup_num)
    ...
    # c3_FPItem, c3_candi, c3_Lminsup = client_episodes_S(seqs[clients.get(3)[0]:clients.get(3)[1]],
    #                                                   timestamp_s[clients.get(3)[0]:clients.get(3)[1]], 130*sup_num)
