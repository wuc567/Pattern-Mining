...
'''i连接'''
def ibacktracking(seqs, timestamp_list, fp, sort_item, Lminsup, FPItem, candi):
    if fp.sup == 0:
        j = 0
        while j < len(seqs):
            # newsup(timestamp_list, j, fp)
            addlist = no_que(timestamp_list, j, seqs[j], fp)
            j += 1
        # if fp.data == [['c'], ['c'], ['a'], ['a', 'd']]:
        #     print('aaaaaa', fp.data, fp.sup)
        init_bool(seqs)
    if fp.sup < Lminsup:
        return
    elif not isincandi(fp.data, FPItem):
            FPItem.append(fp)
    if len(fp.data) == 1 and type(fp.data[0]) == str:
        i = sort_item.index(fp.data[-1])+1
    else:
        i = sort_item.index(fp.data[-1][-1])+1
    while i < len(sort_item):
        if type(fp.data[-1]) == str:
            lastidx = sort_item.index(fp.data[-1])
        else:
            lastidx = sort_item.index(fp.data[-1][-1])

        if i > lastidx:
            temp = copy.deepcopy(fp.data)  # I-连接
            if type(temp[-1]) == str:
                temp.append(sort_item[i])
                tp = Candilist([temp])
            else:
                temp[-1].append(sort_item[i])
                tp = Candilist(temp)
            candi.append(tp)
            ibacktracking(seqs, timestamp_list, tp, sort_item, Lminsup, FPItem, candi)
        i += 1

'''s连接'''
def sbacktracking(seqs, timestamp_list, fp, sort_item, Lminsup, FPItem, candi):
    if fp.sup == 0:
        j = 0
        while j < len(seqs):
            # newsup(timestamp_list, j, fp)
    ...
    while i < len(sort_item):
        ...
        tmp = Candilist(temp)
        candi.append(tmp)
        sbacktracking(seqs, timestamp_list, tmp, sort_item, Lminsup, FPItem, candi)
        i += 1

'''深度优先生成候选模式 DFjion+isincan+DFjionin'''
def DFjoin(seqs, timestamp_list, sort_item, candi, FPItem, FP, Lminsup):
    FPnum = []
    FPnum.append(len(FPItem))
    while len(FPnum) != 0:
        i = 0
        while i < len(FP):
            ibacktracking(seqs, timestamp_list, FP[i], sort_item, Lminsup, FPItem, candi)
            sbacktracking(seqs, timestamp_list, FP[i], sort_item, Lminsup, FPItem, candi)
            i += 1
        FPnum.append(len(FPItem))
        FP.clear()
        i = FPnum.pop(0)
...
def client_episodes(seqs, timestamp_s, result):
    ...
    DFjoin(seqs, timestamp_s, sort_item, candi, FPItem, FP, Lminsup)
        ...
        if v >= fminsup:
            ...
            if get_result_value(filter_result, k):
                ...
                if sum_rem >= fminsup - v:
                    ...
                    rem_result = []
                    for i in clients_rem:
                        dict1 = input_dict.get(i)
                        rem_val = get_sub_value(dict1, k)
                        # 如果等于-1，说明不需要考虑size为1情节，没有前后缀子情节，直接查询。
                        if rem_val == -1:
                            break
                        else:
                            rem_result.append(rem_val)
                    if len(rem_result) == len(clients_rem):
                        rem_sup_upper = sum(rem_result)
                        if rem_sup_upper + v < fminsup:
                            continue
                        else:
