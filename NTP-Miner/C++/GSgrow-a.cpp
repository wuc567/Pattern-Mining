		...
		{	
			...
			for(int j = 0;j<IPLUS[i].poset.size();j++)
			{
				...
				if(len == sub_ptn.size())
				{
					...
					if(max <= b)
					{
						for(int h=max+1;h<a;h++)
						{
							if(belong(sDB[i].S[h],s))
							{
								flag=1;
								break;
							}
