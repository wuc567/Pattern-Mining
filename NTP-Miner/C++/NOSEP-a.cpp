...
char s[10]="hilkmftwv";
  ...
  char mw[30]="hilkmftwvrcqgpsynadeuox";
char sm[30]="hilkmftwvrcqgpsynadeuox";
char m[30]="hilkmftwvrcqgpsynadeuox";
		...
		for (int j=0;j<ptn_len;j++)
		{
			if (sub_ptn[j].end==S[i])
			{
				...
				for (k=start[j];k<prev_len;k++)
				{
					...
					bool flag=false;
					for(int f=nettree[j][k].name+1;f<i;f++)
					{
						if(belong(S[f],s))
						{
							flag=true;
...
void read_file()
{
	...
	string buff;	
	file.open("F:/NTP-Miner/NTP-DataSet/SDB6.txt",ios::in);   //open sequence file
	int i=0;
