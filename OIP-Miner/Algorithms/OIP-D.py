...
def dfs():
    for pa in longcandi:
        depthfirst(pa,oneitem)
    longcandi.clear()

# 深度优先方式生成模式
def depthfirst(pa,oneitem):
    global cannum
    key_tuple = tuple(map(tuple, pa))
    ...
    for item in oneitem:
        ...
        if tmp[0][0] not in SList.keys():
            flag = False
        elif item not in SList[tmp[0][0]]:
            flag = False
        if flag == True:
            tmp.append([item])
            cannum += 1
            ...
            if support >= minsup:
                ...
                if support>=minsupsum:
                    ...
                    depthfirst(tmp, oneitem)
        if item > pa[-1][-1]:
            tmp = copy.deepcopy(pa)
            if len(tmp) == 1:
                flag1 = True
                if item not in IList[tmp[0][0]]:
                    continue
            else:
                flag1 = False
                if item not in SList[tmp[0][0]]:
                    continue
            tmp[-1].append(item)
            cannum += 1
            if flag1 == True:
