...
def singleposition(pattern,dataTable):
    ...
    patternAllPosition = []
    itempos = {}
    lst = sorted(list(set(sum(pattern,[]))))
    for ps in dataTable[lst[0]].keys():
        point = []
        for i in range(len(pattern)):
            point.append(0)
            if ps in dataTable[pattern[i][0]].keys():
                patternpos = dataTable[pattern[i][0]][ps][:]
            else:
                patternAllPosition.clear()
                break
            if len(pattern[i]) > 1:
                for j in range(1, len(pattern[i])):
                    if ps in dataTable[pattern[i][j]].keys():
                        patternpos = inter(patternpos, dataTable[pattern[i][j]][ps][:])
                    else:
                        patternpos=[]
                        break
            if patternpos != []:
                patternAllPosition.append(patternpos)
            else:
                patternAllPosition.clear()
                break
...
# 读文件
def readfile(file):
    global minsup,dataTable,itemsetnum
