...
double r = 0.06;
int k = 22;
string file = "d:/组内算法/pattern-mining-master/scp-miner/dataset/location.txt";
...
void readfile()
{
	...
	string buf;
	while (ifs >> buf) {
		totalseq.push_back(move(buf)); // 使用移动语义避免不必要的复制
	}
	ifs.close();
	for (int i = 0; i < 1; i++) {
		if (totalseq[i][0] >= 65) {
...
double Top_k_Pos(double r, int k) {
	...
	for (auto i = pattern.begin(); i != pattern.end(); i++) {
		...
		if (F1.size() >= k)
			value = (*i).RatePos - F1.top().Contrast;//只有大于thre = seqsize * value，才会被加入进来
		...
		if (i->RatePos > 0) {//正向最小支持率剪枝策略
			...
			candidate_count++;
			E.push_back(*i);
			//cout << (*i).P << ":" << (*i).RatePos << "-" << (*i).RateNeg << "=" << (*i).Contrast << endl;
		}
	...
	while (!G.empty()) {//只要候选模式集合不为空，就用它进行枚举
		...
		for (auto j = E.begin(); j != E.end(); j++) {
			...
			if (q.Contrast <= p.Contrast) {
				G.push(q);//先加入到候选模式中
				candidate_count++;
				//cout << q.P << ":" << q.RatePos << "-" << q.RateNeg << "=" << q.Contrast << ";" << F1.top().Contrast << endl;
			}
			else {
				G.push(q);//先加入到候选模式中
				candidate_count++;
...
int main()
{
	...
	while (!F.empty()) {
		for (int i = 1; i <= 22; i++) {
			cout << "string pattern" << i << "=" << "\"" << F.top().P << "\"" << "; "  << endl;
