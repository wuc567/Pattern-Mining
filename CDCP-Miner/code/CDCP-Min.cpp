...
//			if ((F1.size() >= k && q.RatePos <= F1.top().Contrast)) {
...
//		(*i).RateNeg = CalcPatternRate1((*i).P, negseq, negindex, r, value);
//
//		if (i->RateNeg > pos_minContrast) {//负向支持率大于正向最小对比度才能加入进来
//			G.push(*i);
//			candidate_count++;
//		}
//		/*if (i->RateNeg > 0) {
...
//
//			q.RateNeg = CalcPatternRate1(q.P, negseq, negindex, r, value);
//			if (q.RateNeg <= pos_minContrast || (F2.size() >= k && q.RateNeg <= F2.top().Contrast)) {
//				continue;
//			}
//			if (F2.size() >= k)
