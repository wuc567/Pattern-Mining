...
//struct Pattern {
//	string P;
//	double RatePos; // 正向支持率
//	double RateNeg; //负向支持率
//	double Contrast;//对比度
//	vector<int> FirstLocPos;
//	vector<int> FirstLocNeg;
...
//
//double CalcPatternRate1(Pattern& _pattern, vector<string>& sequenceSet, vector<inverted_index*>& indexset, double& r, double& value, vector<int>& FirstLoc, bool flag);
//
...
//		/*temp.FirstLocPos.resize(posseq.size(), 0);
//		temp.FirstLocNeg.resize(negseq.size(), 0);*/
//		temp.FirstLocPos = std::vector<int>(posseq.size(), 0);
//		temp.FirstLocNeg = std::vector<int>(negseq.size(), 0);
...
//}
//
//double  CalcPatternDensity(string& pattern, string& sequence, inverted_index* index, double& r, int FirstLoc, int& LocOut)//计算密度
//{
//	int support = 0; //支持度，p在s中出现的次数
//	int* s = new int[pattern.length()]();//记录每个字符的位置数组
//	memset(s, 0, sizeof(int) * pattern.length());
//
//	int pos = 0;//都没找到
...
//		double temp_r;
//		int LocOut = 0;
//		temp_r = CalcPatternDensity(pattern, sequenceSet[i], indexset[i], r, FirstLoc[i], LocOut);
//		if (flag) {
//			_pattern.FirstLocPos[i] = LocOut;
//		}
//		else
//		{
//			_pattern.FirstLocNeg[i] = LocOut;
//		}
//
...
//		double value = 1;
//		int temp;
//		(*i).RatePos = CalcPatternRate1((*i), posseq, posindex, r, value, (*i).FirstLocPos, true);
//
//		if (F1.size() >= k)
//			value = (*i).RatePos - F1.top().Contrast;
//
//		(*i).RateNeg = CalcPatternRate1((*i), negseq, negindex, r, value, (*i).FirstLocNeg, false);
...
//			Pattern q;
//			q.P = p.P + j->P;
//			q.FirstLocPos = std::vector<int>(p.FirstLocPos);
//			double  value = 1;
//			q.RatePos = CalcPatternRate1(q, posseq, posindex, r, value, p.FirstLocPos, true);
...
//			q.FirstLocNeg = std::vector<int>(p.FirstLocNeg);
//			q.RateNeg = CalcPatternRate1(q, negseq, negindex, r, value, p.FirstLocNeg, false);
...
//		double value = 1;
//		/*(*i).FirstLocNeg.clear();*/
//		(*i).FirstLocNeg.resize(negseq.size(), 0);
//
//		(*i).RateNeg = CalcPatternRate1((*i), negseq, negindex, r, value, (*i).FirstLocNeg, false);
//
...
//
//		/*(*i).FirstLocPos.clear();
//		(*i).FirstLocPos.resize(posseq.size(), 0);*/
//
//		(*i).RatePos = CalcPatternRate1((*i), posseq, posindex, r, value, (*i).FirstLocPos, true);
...
//			Pattern q;
//			q.P = p.P + j->P;
//			q.FirstLocNeg = vector<int>(p.FirstLocNeg);
//			double value = 1;
//
//			q.RateNeg = CalcPatternRate1(q, negseq, negindex, r, value, p.FirstLocNeg, false);
...
//
//			q.FirstLocPos = vector<int>(p.FirstLocPos);
//			q.RatePos = CalcPatternRate1(q, posseq, posindex, r, value, p.FirstLocPos, true);
