...
def prefixSpan(dataList, minsup = 2):       #prefixSpan流程
    ...
    for x in range(lengthFreElem):
        print(f"正在挖掘以频繁项 '{freElem[x]}' 为前缀的序列... (进度: {x + 1}/{lengthFreElem})")
...
if __name__ == '__main__':
    ...
    for x in q:   #输出
        print(x,'::', q[x])
        FPcount += len(q[x])
    print('The number of FPs:' + str(FPcount))
    print('The number of CPs:' + str(count))
