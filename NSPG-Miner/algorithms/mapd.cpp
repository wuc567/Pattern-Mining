...
int get_patlen(Qpattern PT) {
	int length = PT.pattern.length();
	char *S = new char[length];
	strcpy(S, PT.pattern.c_str());
	int patlen = 0;
...
void mineFrePat(int strLen, int n, double threshold, double *offSup, int **SDB, float *pro) {
	...
	f_num = getLen_1Pats(SDB, sup, threshold, buff, n, strLen);
	int stackLen = frePats.size();
	vector <MLin> tempArray;
	vector <node> **newLevels = (vector <node> **)new vector <node> *[NumbS];
	for (int i = 0; i < NumbS; i++)
	{
	...
	while (!frePats.empty())
	{
		...
		for (int t = 0; t < setlen; t++)
		{
			...
			if (sup[t] >= candiSup) {
				...
				if (queueCaptity > stackLen) {
					stackLen = queueCaptity;
				}
			}
			else {
				for (int j = 0; j < NumbS; j++)
