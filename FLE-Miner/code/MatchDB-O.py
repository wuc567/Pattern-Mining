...
def no_que(timestamp_dict, n, bool_s, seq, pattern, client_i):
    addlist = []
    usearray = []  # 标记本轮已用结点
    sflag = 1  # 判断是否终止循环
    k = 0
        ...
        while m < len(pattern.data) and pflag:
            while l < len(seq):
                if pattern.data[m][0] == seq[l].data[0] and bool_s[l]:
                    if m != 0:
                        # 如果不满足时间约束，应该回溯
                        gaptime_flag = time_ok(timestamp_dict.get(client_i), n, usearray[-1], l)
                        #
...
def mainfun():
    ...
    starttime = time.time()
    sup_client = onepattern(seqs, neu, cle, client)  # 1序列模式及位置索引
    ...
    while 1:
        ...
        while i < len(candi):
            ...
            for c, v in seqs_dict.items():
                ...
                j = 0
                while j < len(v):
                    bool_s = init_bool(v[j])
                    addlist = no_que(timestamp_dict, j, bool_s, v[j], candi[i], c)
