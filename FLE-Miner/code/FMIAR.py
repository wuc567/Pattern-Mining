                ...
                for row in reader:
                    if i == 0:
                        ...
                        if day_identity != row[1]:
                            if day_identity != 0:
                                trace_day[day_identity] = [copy.deepcopy(activity_list), copy.deepcopy(timestamp_list), copy.deepcopy(cost_list)]
                            day_identity = row[1]
                            activity_list.clear()
                            timestamp_list.clear()
                            activity_list.append(row[5])
                            timestamp_list.append(parser.parse(row[4]))
                            cost_list.append((parser.parse(row[4])-parser.parse(row[3])).total_seconds())
                        else:
                            activity_list.append(row[5])
                            timestamp_list.append(parser.parse(row[4]))
                            cost_list.append((parser.parse(row[4])-parser.parse(row[3])).total_seconds())
...
def data_preprocess(trace_per):
    ...
    for vals in trace_per.values():
        for svals in vals.values():
            ...
            cost_list.append(svals[2])

    # if len(timestamp_t) > 0:
    #     interval = e['time:timestamp'] - timestamp_t[-1]
...
def write_txt(addr, len_line):
    base_u = []
    for siz in len_line:
        base_u.append(np.random.randint(2, 5, size=siz))
...
def one_to_two():
    ...
    while i < len(FP):
        ...
        j = 0
        aft_candi_num = aft_candi_num + len(FP) + 1
        while j < len(FP):
            ...
            j += 1
        i += 1
    return bef_candi_num, aft_candi_num
    ...
    for i in range(len(FP)):
        for j in range(len(FP)):
            if operator.eq(FP[i].prefix, FP[j].suffer):
                ...
                candi.append(Candilist(tmp))
                cur_candi.append(tmp)
    FP.clear()  # 每次生成完候选模式清空频繁项集
    bef_candi_num, aft_candi_num = computer_candi_num(cur_candi)
    return bef_candi_num, aft_candi_num

# 计算压缩前后候选情节的事件数
def computer_candi_num(cur_candi):
    groups = defaultdict(list)

    for c in cur_candi:
        # 取候选的前 len(c)-1 项作为分组键
        key = tuple(tuple(item) for item in c[:-1])  # 将嵌套列表转换为可哈希的元组
        groups[key].append(c[-1])  # 收集最后一个项

    # 生成合并后的候选集
    candi_comp = []
    for key, values in groups.items():
        # 将元组键还原为列表格式
        merged = [list(item) for item in key]
        # 添加所有合并的末项
        for v in values:
            merged.append(list(v))
        candi_comp.append(merged)

    # 计算候选数量
    bef_candi_num = sum(len(c) for c in cur_candi)  # 12*3=36
    aft_candi_num = sum(len(cc) for cc in candi_comp)  # 7+4+7=18
...
'''
dict1 = {'client_1':[{'a':[4,7,2], 'b':[6,5, 3]}]
 'client_2':[{'a':[8,12,4], 'b':[9,6,6], 'c':[19,36,6]}]
}
dict2 = {'client_1':[{'a':7, 'b':5}]
 'client_2':{'a':12, 'b':6, 'c':36}
}
'''
def client_candi_computer(dict1):
    dict2 = {}
    for client, data_list in dict1.items():
        # 提取客户端数据（假设列表中只有一个字典）
        original_dict = data_list[0]  # 取列表的第一个元素
        # 创建新字典：键转为元组，值取列表的第二个元素（索引1）
        new_dict = {}
        for key, values in original_dict.items():
            tuple_key = ((key,),)  # 将字符串键转为元组，例如 'a' → ('a',)
            new_value = values[1]  # 取列表中第二个元素
            new_dict[tuple_key] = new_value
        dict2[client] = new_dict
    return dict2


'''
ooc_client = {'client_1':{'seq_1':[[0,2,3], [1,4], [3,4]], 'seq_2':[[0,1], [2,5]]}, 'client_2':{'seq_1':[[0,3], [1,4]]}}
dict1 = {'client_1':{('a',):7, ('b',):5}, 'client_2':{('a',):12, ('b',):6, ('c',):36}}
p = [['a'], ['c']]
dict2 = {'client_1':{('a',):7, ('b',):5, (('a',), ('c',)):len(ooc_client['client_1']['seq_1'])+len(ooc_client['client_1']['seq_2'])},
 'client_2':{('a',):12, ('b',):6, ('c',):36, (('a',), ('c',)):len(ooc_client['client_2']['seq_1'])}}
输入为 ooc_client，dict1 和 p ，dict2，使用python代码实现
'''
def add_candi_client(ooc_client, client_candi_sup, p):
    # 将 p 转换为元组键格式
    p_key = tuple(tuple(sub) for sub in p)
    # 为每个客户端添加新的键值对
    for client in client_candi_sup:
        # 计算该客户端所有序列的总长度
        if client in ooc_client:
            total_length = sum(len(seq) for seq in ooc_client[client].values())
        else:
            total_length = 0  # 默认值
        # total_length = sum(len(seq) for seq in ooc_client[client].values())
        # 添加合并后的键值对
        client_candi_sup[client][p_key] = total_length
...
def mainfun():
    ...
    # 数据集1 csv文件
    # trace_per = read_csv("../dataset/daily living/")
    # activity_chart_r, timestamp_list_r, cost_list_r = data_preprocess(trace_per)
    ...
    # write_txt('../dataset/SDB3_NoExt.txt', len_line)
    # 读取效用文件
    ...
    client_candi_sup = client_candi_computer(sup_client)
    # print(client_candi_sup)
    ...
    # 压缩前后候选情节事件数量
    bef_comp_num, aft_comp_num = one_to_two()  # 从FP中生成存到candi
    # sandilinked()  # 1->2
    # print(candi)
    candinum.append(len(candi))
    FP.clear()
    while 1:
        ...
        while i < len(candi):
            ...
            for c, v in clientNext.items():
                # 如果候选情节的前缀或后缀在该客户端的支持度为0或不存在则continue
                client_data = client_candi_sup.get(c, [{}])  # 若不存在则默认空字典
                iscomp_pre = client_data.get(tuple(tuple(sub) for sub in candi[i].data[:-1]), 0)  # 前缀存在返回对应值，否则返回0
                iscomp_suf = client_data.get(tuple(tuple(sub) for sub in candi[i].data[1:]), 0)  # 后缀存在返回对应值，否则返回0
                # print(candi[i].data, tuple(tuple(sub) for sub in candi[i].data[:-1]), iscomp_pre, tuple(tuple(sub) for sub in candi[i].data[1:]), iscomp_suf)
                #
                if iscomp_pre == 0 or iscomp_suf == 0:
                    candi_comp_num += 1
                    continue
                ooc_dict = dict()
                j = 0
            ...
            # 计算平均成本(eac)和成本下限（lbc）
                ...
                if eac <= maxeac:
                    ...
                    FP.append(candi[i])
                    add_candi_client(ooc_client, client_candi_sup, candi[i].data)
                    # print(client_candi_sup)
                elif lbc <= maxeac:
                    FP.append(candi[i])
                    add_candi_client(ooc_client, client_candi_sup, candi[i].data)
                    # print(client_candi_sup)
                # if candi[i].sup >= mineau:
                #     FP.append(candi[i])
                #     FPItem.append(candi[i])
            i += 1

        bef_candi_num, aft_candi_num = sandilinked()
        bef_comp_num = bef_comp_num + bef_candi_num
        aft_comp_num = aft_comp_num + aft_candi_num
        # FP.clear()
        if cannum == len(candi):
            break
        else:
    ...
    print("候选情节压缩前：", bef_comp_num)
    print("候选情节压缩后：", aft_comp_num)
    print("少传输候选情节数：", candi_comp_num)
