...
'''s连接'''
def sbacktracking(timestamp_dict, neu_dict, cle_dict, fp, sort_item, sup_client):
    if fp.sup == 0:
        # 再求1长度支持度时，模式应该是[['01_BB_540']],而不是['01_BB_540']，故加一层[]
        ...
        for c, v in clientNext.items():
            ...
            j = 0
            while j < len(v):
                addlist = newsup(timestamp_dict, j, fp, c)
                ooc_dict[j] = addlist
                j += 1
            ooc_client[c] = ooc_dict

        allconf = calculate_all_conf(ooc_client, sup_client, fp.data)
        if allconf < mincor:
            return
        else:
            ...
            if lbc > maxeac:
                return
            if eac <= maxeac and not (len(fp.data) == 1 and type(fp.data[0]) == str):
    ...
    while i < len(sort_item):
        ...
        tmp = Candilist(temp)
        candi.append(tmp)
        sbacktracking(timestamp_dict, neu_dict, cle_dict, tmp, sort_item, sup_client)
        i += 1

'''深度优先生成候选模式 DFjion+isincan+DFjionin'''
def DFjoin(timestamp_dict, neu_dict, cle_dict, sort_item, sup_client):
    FPnum.append(len(FPItem))
    while len(FPnum) != 0:
        i = 0
        while i < len(FP):
            sbacktracking(timestamp_dict, neu_dict, cle_dict, FP[i], sort_item, sup_client)
            i += 1
        FPnum.append(len(FPItem))
        FP.clear()
        i = FPnum.pop(0)
...
def mainfun():
    ...
    DFjoin(timestamp_dict, neu_dict, cle_dict, sort_item, sup_client)
    endtime = time.time()
