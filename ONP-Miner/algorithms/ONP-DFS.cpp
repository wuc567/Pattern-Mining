...
int frennum = 0; int pfrenum = 0; int nfrenum = 0;

int matchp(gapnit* p, char* s);

int pf = 0;
void mineFre(string p)
{
	...
	for (int m = 0; m < size; m++)
	{
		...
		string c;
		c = freArr[0][m];
		q = p + '[' + char(48 + mins) + ',' + char(48 + maxs) + ']' + c;
		pfrenum++;
		int candsup;
		candsup = matchsup(q);
		if (candsup >= threshold)
		{
			...
			else
			{
				...
				for (int j = 0; j < setlen; j++)
				{
					string e;
					e = charset[j];
					string m = p + '[' + char(48 + mins) + ',' + char(48 + maxs) + ']' + '-' + charset[j] + c;
					pfrenum++;
					int candsup = matchsup(m);
					if (candsup >= threshold)
					{
						frennum++;
						mineFre(m);
		...
		else
		{
			for (int j = 0; j < setlen; j++)
			{
				string e;
				e = charset[j];
				q = p + '[' + char(48 + mins) + ',' + char(48 + maxs) + ']' + '-' + charset[j] + c;
				pfrenum++;
				int candsup = matchsup(q);
				if (candsup >= threshold)
				{
					frennum++;
					mineFre(q);
