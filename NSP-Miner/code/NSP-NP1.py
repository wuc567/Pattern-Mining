...
def Matching_S(list1, list2):
    ...
    for i in range(SeqNum):
        ...
        while tmp1>0 and tmp2>0:
            ...
            tmp1 >>= 1
            #print(bin(tmp1))
            # if tmp2 > tmp1 and tmp1 & tmp2 !=0 and (tmp1 | tmp2) % tmp2 == tmp1:
            if tmp1.bit_length() < (tmp2 & -tmp2).bit_length():
            # __pos1 = bin(tmp1)[2:].zfill(len(bin(tmp1)[2:])).index("1")
            # __pos2 = bin(tmp2)[2:][::-1].zfill(len(bin(tmp2)[2:])).index("1")
            # if __pos1 > __pos2:
                break
