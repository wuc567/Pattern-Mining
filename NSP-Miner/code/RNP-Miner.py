...
def Miner():
    ...
    for i in range(len(FP)):
        PattanSeqSupdic[str(FP[i])] = []
        # print(FP[i])
        for j in range(0, SeqNum):
            PattanSeqSupdic[str(FP[i])].append(len(ItemS[str(FP[i])][j]))
            # if len(ItemS[str(FP[i])][j])>0:
            #     PattanSeqSupdic[str(FP[i])].append(1)
            # else:
            #     PattanSeqSupdic[str(FP[i])].append(0)
            # print(len(ItemS[str(FP[i])][j]))
        # print(str(FP[i]) + ' ' + str(ItemS[str(FP[i])]))
...
if __name__ == '__main__':
    ...
    for minsup in sys.argv[2:]:
        ...
        pdataFrame = pd.DataFrame(PattanSeqSupdic)
        # pdataFrame = pdataFrame.T  # 旋转
        pdataFrame.to_excel(readFileName + "rnp.xlsx", sheet_name='sheet1')